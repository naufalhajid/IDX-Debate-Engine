...
class ChatCodexResponses(BaseChatModel):
    ...
    def _format_messages(self, messages: List[BaseMessage]) -> tuple[str, List[Dict[str, Any]]]:
        ...
        for msg in messages:
            if isinstance(msg, SystemMessage):
                ...
                if isinstance(content, str):
                    inputs.append({"role": "user", "content": [{"type": "input_text", "text": content}]})
                else:
                ...
                if msg.content:
                    inputs.append({"role": "assistant", "content": [{"type": "output_text", "text": str(msg.content)}]})
    ...
    def _build_api_kwargs(self, messages: List[BaseMessage], **kwargs: Any) -> Dict[str, Any]:
    ...
    async def _astream(
        ...
        **kwargs: Any,
        ...
        async for event in stream:
            ...
            if event_type == "error":
                raise RuntimeError(f"Responses API error: {getattr(event, 'message', '')}")
            ...
            if event_type == "response.output_item.done":
                ...
                if itype == "function_call":
                    ...
                    call_id = item.get("call_id", "") if isinstance(item, dict) else getattr(item, "call_id", "")
            ...
            if event_type in {"response.completed", "response.failed"}:
                ...
                if getattr(resp_obj, "error", None) or (isinstance(resp_obj, dict) and resp_obj.get("error")):
