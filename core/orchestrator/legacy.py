...
def _pre_cio_terminal_result(
    ...
    *,
    ...
    signal_packet = set_forecast_state(
        signal_packet,
        "SKIPPED_PRE_CIO",
    )
    metadata["signal_packet"] = signal_packet
    from services.recommendation_context import (
        build_terminal_recommendation_context,
    )

    metadata["recommendation_context"] = build_terminal_recommendation_context(
        reason_code=reason_code,
        reason=reason,
...
def _finalize_execution_decisions(results: list[dict]) -> None:
    ...
    for entry in results:
        ...
        try:
            ...
            recommendation_context = finalize_recommendation_context(
                metadata.get("recommendation_context"),
                decision=decision,
                result=entry,
            )
            metadata["recommendation_context"] = recommendation_context
            entry["recommendation_context"] = recommendation_context
            entry["recommendation_state"] = recommendation_context.get(
                "recommendation_state"
            )
        except Exception as exc:
            logger.warning(
                "[RecommendationContext] {} finalization failed: {}",
                entry.get("ticker"),
                exc,
            )
        for field in (
...
def build_execution_funnel(results: list[dict]) -> dict[str, Any]:
    ...
    for entry in results:
        ...
        recommendation_context = entry.get("recommendation_context")
        if not isinstance(recommendation_context, dict):
            recommendation_context = metadata.get("recommendation_context")
        if not isinstance(recommendation_context, dict):
            if execution_status == "EXECUTABLE_BUY":
        ...
        recommendation_state = str(
            recommendation_context.get("recommendation_state")
            or (
                "DATA_INSUFFICIENT"
                if execution_status == "INSUFFICIENT_DATA"
                else "HARD_REJECT"
            )
        ).upper()
        recommendation_state_counts[recommendation_state] = (
            recommendation_state_counts.get(recommendation_state, 0) + 1
        )
        blockers = recommendation_context.get("blockers")
        ...
        ticker_outcomes.append(
            {
                ...
                "blockers": blockers,
                "next_observable_trigger": recommendation_context.get(
...
def _print_top3_summary(
    ...
    if not top_n:
        ...
        console.print(
            Panel(
                (
                    "[warn]No executable BUY setups.[/warn] Non-executable "
                    "candidates remain visible below by recommendation state; "
                    "none permits sizing."
                ),
        ...
        from app.cli.ui.tables import build_recommendation_diagnostics_tables

        for diagnostics in build_recommendation_diagnostics_tables(all_results or []):
