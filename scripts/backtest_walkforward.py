...
def run_backtest(
    ...
    for ticker in tickers:
        ...
        for h in horizons:
            ...
            logger.info(
                "[%s] h=%2dd  IC=%.3f  t=%.2f  dir=%.2f  status=%s",
                ticker, h,
                vs.ic_mean or 0.0,
                vs.ic_t_stat or 0.0,
...
def main() -> None:
    ...
    print(f"\n{'TICKER':<12} {'H':>3} {'N_OBS':>6} {'IC':>7} {'T-STAT':>7} {'DIR':>6} {'STATUS'}")
    print("-" * 60)
    for r in rows:
        print(
            f"{r['ticker']:<12} {r['horizon_d']:>3} {r['n_obs']:>6} "
            f"{r['ic_mean']:>7} {r['ic_t_stat']:>7} {r['dir_acc']:>6} {r['status']}"
        )
