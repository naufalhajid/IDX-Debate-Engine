...
# Consensus evaluation and state cleaning are deterministic Python nodes.
# Their historical prompt files remain archived in services/debate_prompts.
...
"""
    ...
    async def _consensus_evaluator_node(self, state: DebateChamberState) -> dict:
        ...
        Deterministic: this node must not call an LLM or require archived prompts.
        """
    ...
    def _tick_size_for_price(price: float) -> float:
        return TradeEnvelopeService.tick_size_for_price(price)

    @classmethod
    def _next_tick_above(cls, price: float) -> float:
        return TradeEnvelopeService.next_tick_above(price)

    @classmethod
    def _previous_tick_below(cls, price: float) -> float:
        return TradeEnvelopeService.previous_tick_below(price)

    MAX_TARGET_RETURN = TradeEnvelopeService.MAX_TARGET_RETURN
    _SECTOR_MAX_TARGET = TradeEnvelopeService.SECTOR_MAX_TARGET
    _SECTOR_ALIAS = TradeEnvelopeService.SECTOR_ALIAS

    @classmethod
    def _max_target_return(cls, sector: str | None) -> float:
        return TradeEnvelopeService.max_target_return(sector)
    ...
    def _compute_trade_envelope(
        ...
        """
        return TradeEnvelopeService.compute(
            current_price=current_price,
