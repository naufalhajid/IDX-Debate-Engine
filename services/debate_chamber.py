...
from services.recommendation_context import build_setup_recommendation_context
...
"""
    ...
    def _run_tradeability_preflight(tech: "dict | None", current_price: float) -> dict:
        ...
        if surrogate_gap < hard_floor:
            ...
            return {
                ...
                "hard_noise_atr_multiplier": (
                    settings.TRADE_ENVELOPE_HARD_NOISE_ATR_MULTIPLIER
                ),
                "clean_noise_atr_multiplier": (
                    settings.TRADE_ENVELOPE_CLEAN_NOISE_ATR_MULTIPLIER
                ),
                "gate_diagnostics": [
                    {
                        "gate_id": "stop_noise_floor",
                        "hard_or_soft": "HARD",
                        "reason_code": "preflight_noise_reject",
                        "observations": [
                            {
                                "name": "price_to_swing_low_gap",
                                "observed": surrogate_gap,
                                "threshold": hard_floor,
                                "comparator": ">=",
                                "unit": "IDR",
                                "absolute_gap": gap,
                                "percentage_gap": gap / hard_floor,
                            }
                        ],
                        "provenance": (
                            "services.debate_chamber.DebateChamber."
                            "_run_tradeability_preflight"
                        ),
                        "detail": reason,
                        "next_observable_trigger": (
                            "Wait until the price-to-swing-low gap is at least "
                            f"Rp {hard_floor:,.0f}, then recompute all gates."
                        ),
                    }
                ],
            }
    ...
    def _compute_trade_envelope(
        ...
        if rejections:
            ...
            for failure_code, failure_reason in rejections:
                observations: list[dict[str, Any]] = []
                blocker_class = "HARD"
                trigger: str | None = None
                gate_id = failure_code
                if failure_code == "rr_too_low":
                    blocker_class = "SOFT"
                    gate_id = "risk_reward_floor"
                    rr_gap = max(0.0, rr_requirement.required_rr - rr_ratio)
                    observations.append(
                        {
                            "name": "risk_reward_ratio",
                            "observed": rr_ratio,
                            "threshold": rr_requirement.required_rr,
                            "comparator": ">=",
                            "unit": "x",
                            "absolute_gap": rr_gap,
                            "percentage_gap": (
                                rr_gap / rr_requirement.required_rr
                                if rr_requirement.required_rr > 0
                                else None
                            ),
                        }
                    )
                elif failure_code == "stop_inside_noise":
                    gate_id = "stop_noise_floor"
                    stop_gap = max(0.0, _hard_floor_atr - _stop_distance)
                    observations.append(
                        {
                            "name": "stop_distance",
                            "observed": _stop_distance,
                            "threshold": _hard_floor_atr,
                            "comparator": ">=",
                            "unit": "IDR",
                            "absolute_gap": stop_gap,
                            "percentage_gap": (
                                stop_gap / _hard_floor_atr
                                if _hard_floor_atr > 0
                                else None
                            ),
                        }
                    )
                elif failure_code == "target_collapsed":
                    gate_id = "target_geometry"
                    target_gap = max(0.0, entry_high - target)
                    observations.append(
                        {
                            "name": "target_price",
                            "observed": target,
                            "threshold": entry_high,
                            "comparator": ">",
                            "unit": "IDR",
                            "absolute_gap": target_gap,
                            "percentage_gap": (
                                target_gap / entry_high if entry_high > 0 else None
                            ),
                        }
                    )
                elif failure_code == "momentum_breakdown":
                    observations.extend(
                        [
                            {
                                "name": "return_5d",
                                ...
                                "comparator": ">",
                                "unit": "%",
                                "absolute_gap": max(0.0, -3.0 - float(return_5d)),
                                "percentage_gap": None,
                            },
                            {
                                "name": "close_vs_ema20",
                                ...
                                "unit": "IDR",
                                "absolute_gap": max(
                                    0.0, float(ema20) - current_price
                                ),
                                "percentage_gap": (
                                    max(0.0, float(ema20) - current_price)
                                    / float(ema20)
                                    if float(ema20) > 0
                                    else None
                                ),
                    ...
                    trigger = (
                        "Wait until five-day return is above -3% or close is "
                        "at/above EMA20; then recompute every execution gate."
                    )
                gate_diagnostics.append(
                    {
                        "gate_id": gate_id,
                        "hard_or_soft": blocker_class,
        ...
        if wait_for_confirmation:
            ...
            if ema20 is None or current_price < ema20:
                close_gap = (
                    max(0.0, float(ema20) - current_price)
                ...
                confirmation_observations.append(
                    {
                        ...
                        "comparator": ">=",
                        "unit": "IDR",
                        "absolute_gap": close_gap,
                        "percentage_gap": (
                            close_gap / float(ema20)
                            if close_gap is not None and float(ema20) > 0
                            else None
                        ),
                    }
                )
            if return_1d is None or return_1d <= 0.0:
                confirmation_observations.append(
                    {
                        "name": "return_1d",
                        "observed": return_1d,
            ...
            if volume_ratio is None or volume_ratio < 1.0:
                volume_gap = (
                    max(0.0, 1.0 - float(volume_ratio))
                ...
                confirmation_observations.append(
                    {
                        ...
                        "comparator": ">=",
                        "unit": "x",
                        "absolute_gap": volume_gap,
                        "percentage_gap": volume_gap,
                    }
                )
            accepted.update(
                ...
                gate_failures=[
                    {
                        ...
                        "detail": confirmation_trigger,
                    }
                ],
                gate_diagnostics=[
                    {
                        "gate_id": "momentum_confirmation",
                        "hard_or_soft": "SOFT",
                        "reason_code": "wait_for_momentum_confirmation",
                        ...
                        "detail": "Momentum confirmation conditions are incomplete.",
                        "next_observable_trigger": confirmation_trigger,
                    }
                ],
            )
        return accepted
...
Start your response with '{' and end with '}'. Nothing else."""
    ...
    async def _run_scouts(
        ...
        if isinstance(snapshot, dict):
            ...
            metadata["recommendation_context"] = dict(
                snapshot.get("recommendation_context") or {}
            )
    ...
    async def run(
        ...
        recommendation_context = build_setup_recommendation_context(
            snapshot,
            signal_packet=signal_packet,
        )
        snapshot["recommendation_context"] = recommendation_context
        ...
        initial_state: DebateChamberState = {
            ...
            "metadata": {
                ...
                "recommendation_context": dict(recommendation_context),
