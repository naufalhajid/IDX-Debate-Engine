...
def build_trade_setup_snapshot(
    *,
    ...
    def finalize(snapshot: dict[str, Any]) -> dict[str, Any]:
        ...
        snapshot["recommendation_context"] = build_setup_recommendation_context(
            snapshot,
            signal_packet=snapshot["signal_packet"],
        )
        return snapshot
        ...
        return finalize(
            {
                ...
                "gate_diagnostics": list(
                    preflight.get("gate_diagnostics")
                    if isinstance(preflight.get("gate_diagnostics"), list)
                    else []
                ),
            }
        )
    ...
    if envelope.get("rejected"):
        ...
        }.get(envelope_reason, "UNKNOWN_REJECT")
    ...
    return finalize(
        {
            ...
            "gate_failures": list(
                envelope.get("gate_failures")
                if isinstance(envelope.get("gate_failures"), list)
                else []
            ),
            "gate_diagnostics": list(
                envelope.get("gate_diagnostics")
                if isinstance(envelope.get("gate_diagnostics"), list)
                else []
            ),
        }
    )
