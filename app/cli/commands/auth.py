...
from providers.oauth_manager import _lock_path, _read_auth_store, _write_auth_store, file_lock

app = typer.Typer(help="Manage authentication tokens for LLM providers.")

CODEX_OAUTH_CLIENT_ID = "app_EMoamEEZ73f0CkXaXp7hrann"  # Identik dengan Hermes Codex Client ID
...
def _codex_device_code_login() -> Optional[str]:
    ...
    console.print("[idx.muted]Waiting for sign-in... (press Ctrl+C to cancel)[/idx.muted]")
...
def _add_token(provider: str, token: str) -> None:
    ...
    try:
        ...
        console.print(f"   [idx.muted]Anda sekarang bisa menjalankan sistem menggunakan {provider}.[/idx.muted]")
    except Exception as e:
...
def auth_add_command(
    provider: Annotated[str, typer.Argument(help="Nama provider (openai-codex, anthropic, gemini)")],
    token: Annotated[Optional[str], typer.Argument(help="Access Token (opsional untuk codex)")] = None,
) -> None:
    ...
    if provider_raw in ["openai-codex", "codex"]:
        provider_key = "codex"
        if not token:
            console.print("[idx.header]Memulai Device Code Login untuk OpenAI Codex...[/idx.header]")
        ...
        console.print(f"[idx.error]❌ Provider '{provider_raw}' tidak didukung.[/idx.error]")
        raise typer.Exit(code=1)
